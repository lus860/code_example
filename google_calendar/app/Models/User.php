...
class User extends Authenticatable implements JWTSubject, MustVerifyEmail
{
    ...
    {
        $this->avatar = '';
        if ($this->file_path && $this->lms_file == self::LMS_FILE_NOT_ACTIVE) {
            $this->avatar = $this->_getCompany()->getFilePathApm() . $this->file_path;
        } elseif ($this->file_name && $this->lms_file == self::LMS_FILE_ACTIVE) {
            $this->avatar = $this->_getCompany()->getFilePath('userpics') . config('config.directory_separator') . $this->file_name;
        }
        return $this->avatar;
    }
